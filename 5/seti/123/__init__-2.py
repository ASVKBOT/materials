...
server_verbosing = False


def prints(*args, **kvargs):
    if server_verbosing:
        print(*args, **kvargs)
...
def ngettext(*args):
    return LOCALES[curloc].ngettext(*args)
...
class player:
    ...
    def move(self, x, y):
        ...
        if self.y < 0:
            self.y = self.max_y
        print(_("Moved to {} {}").format(self.x, self.y))
...
class monster:
    ...
    def got_hit(self, damage):
        ...
        """
        print(_("Attacked {},  damage {} hp").format(self.cow, damage))
        self.hp -= damage
...
class dungeon:
    ...
    def add_monster(self, x, y, phase, hp, cow="default"):
        if str(self.mas[x][y]) == "M":
            print(_("There is monster already"))
            return
        ...
        if str(self.mas[x][y]) == "M":
            if name == self.mas[x][y].cow:
                ...
                if self.mas[x][y].hp < 0:
                    print(_("{} died").format(self.mas[x][y].cow))
                    self.mas[x][y] = "0"
                    self.monsters.remove((x, y))
                else:
                    print(
                        ngettext(
                            "{} now has {} hp", "{} now has {} hp", self.mas[x][y].hp
                        ).format(self.mas[x][y].cow, self.mas[x][y].hp)
                    )
            else:
                print(_("No {} here").format(name))
        else:
            print(_("No monster here"))
...
class game(cmd.Cmd):
    ...
    def preloop(self):
        prints(_("<<< Welcome to Python-MUD 0.1 >>>"))
        self.dug = dungeon()
        ...
        if len(tokens) != 8:
            ...
            try:
                ...
                if not (monster_name in list_cows()):
                    print(*list_cows(), sep=", ")
                    print(_("Cannot add unknown monster"))
                else:
    ...
    def do_movemonsters(self, args):
        ...
        if args == "on":
            self.enabled_monters = True
            print(_("Wandering monsters enabled"))
        elif args == "off":
            self.enabled_monters = False
            print(_("Wandering monsters disabled"))
        else:
            print(_("Bad argument"))

    def do_locale(self, args):
        """
        Переключает локаль для игрока
        """
        if args in LOCALES.keys():
            global cl_languages
            cl_languages[self.cur_pl] = args
            print(_("Set up locale: {}").format(args))
        else:
            print(_("Unknown locale"))
        ...
        for q in done:
            if q is send:
                ...
                if not (q.result().decode().strip() in logins.values()):
                    ...
                    prints(f"{me} logged in as {q.result().decode().strip()}")
                    logged = True
                    break
                else:
            ...
            continue
        break
    cl_languages.setdefault(logins[me], "en")
    clients[me] = tmp
    global curloc
...
async def main(enable_wander=True):
    if enable_wander:
...
def run_main(enable_wander=True, verbose=True):
    global app
    global server_verbosing
    server_verbosing = verbose
    app = game()
    app.preloop()
    asyncio.run(main(enable_wander))
